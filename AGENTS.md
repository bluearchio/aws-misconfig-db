# AWS Misconfiguration Database - Codex Instructions

## Project Role

This repo is the source of truth for the misconfiguration recommendation
catalog. Governance Hub imports from this repo, while BlueArch CLI and Tag
Manager should not duplicate or expand bundled misconfig catalog copies.

Executable support is not decided here. Keep executable/non-executable mapping
metadata in Governance Hub/core-facing storage, not in ad hoc frontend logic.
...
```
This scans all enabled sources, deduplicates against existing recommendations, converts via Codex API, validates against schema, and stages new candidates in `data/staging/`.
...
- Do not add product-runtime assumptions to catalog rows unless the schema
  explicitly supports them.
- If the ingest pipeline changes generated catalog shape, update Governance Hub
  import expectations in the same feature branch.
- Commit messages should use git-cliff prefixes such as `feature:`, `bugfix:`,
  `fix:`, `chore:`, and `docs:` when this repo participates in the BlueArch
  release flow.
